...
![一次从 HBM 请求数据的流程：SM → L1D（miss）→ L2（miss）→ Crossbar 路由到对应的 Memory Controller → 从 HBM3 Stack 取出 cache line，原路返回并沿途写入 L2 与 L1D](../figure/hbm3_request_flow.gif)
...
![Coalesced access：一个 warp 的地址被合并到尽量少的 sector 中](../figure/fig1.1.png)

![反面例子](../figure/fig1.2.png)
...
![H100 SM 架构：4 个 SMSP（各含 Warp Scheduler、Register File、INT32/FP32/FP64 单元与第四代 Tensor Core），下方为 TMA 和 256KB 的 L1/Shared Memory](../figure/fig1.3.png)
...
![Latency Hiding：某个 warp 等待内存时，Warp Scheduler 立即切换到其他就绪的 warp，SM 因此始终保持忙碌](../figure/latency_hiding.gif)

## 动手实验

本章的四个硬件概念（Coalescing、Occupancy 与 Latency Hiding、Bank Conflict、SFU 吞吐）各配了一个可编译运行的小实验，放在 **[Assignment 01](../assignment/assignment01/)**。读完本章建议亲手跑一遍——纸上的硬件数字变成实测数据，理解会深很多。
